...
function renderPayment() {
  ...
  $('#checkout-content').innerHTML = `<div class="payment-stage"><p class="eyebrow">PAYMENT REQUEST</p><img class="payment-qr-image" src="upi-qr.jpeg" alt="PHOFRAMES UPI QR code"><h3>Your frame is<br>almost home.</h3><p class="payment-copy">Scan the QR code or use your UPI app. The amount is ${money(total)}.</p><div class="upi-id-line"><span>${UPI_ID}</span><button type="button" id="copy-upi">COPY</button></div><a class="primary-button full-button" href="${upiUrl}">PAY NOW IN YOUR UPI APP</a><button class="paid-button" type="button" id="payment-returned">I HAVE COMPLETED PAYMENT</button><p class="manual-note">After payment, share your screenshot on WhatsApp so we can book your order.</p></div>`;
}

function renderCelebration() {
  const total = lineTotal(checkoutLines);
  const message = `Hello PHOFRAMES! I have completed payment of ${money(total)} for my frame order. I am sharing my payment screenshot here.`;
  $('#checkout-title').textContent = 'Thank you';
  $('#checkout-content').innerHTML = `<div class="celebration-stage"><div class="check-orb">✓</div><p class="eyebrow">PAYMENT RECEIPT NEXT</p><h3>Thank you — let’s make it official.</h3><p>Send your payment screenshot to PHOFRAMES on WhatsApp. Once it’s received, we’ll book your frame order.</p><a class="primary-button full-button receipt-link" href="${whatsappLink(message)}">SHARE PAYMENT SCREENSHOT</a></div>`;
}
